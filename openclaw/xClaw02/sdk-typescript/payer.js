...
function parsePaymentRequirements(responseData) {
  ...
  if (responseData.x402Version !== 2) {
    throw new Error(`Unsupported x402Version: ${responseData.x402Version}. Expected version 2.`);
  }
  ...
  const requirements = responseData.accepts[0];
  if (requirements.network && !requirements.network.startsWith('eip155:')) {
    requirements.network = toCaipNetwork(requirements.network);
  }
  return requirements;
}
...
async function createPayment(signer, requirements, facilitator) {
  ...
  // Network is already in CAIP-2 format from signer.getNetwork()
  return {
    x402Version: XCLAW02_VERSION,
    scheme: 'exact',
