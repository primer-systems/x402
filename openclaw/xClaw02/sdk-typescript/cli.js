...
// xClaw02 - Command Line Interface
...
COMMANDS:
  ...
  probe <url>                Check if URL supports xclaw02 payments
...
OPTIONS:
  ...
  --private-key <key>        Private key (or use XCLAW02_PRIVATE_KEY env)
...
EXAMPLES:
  ...
  xclaw02 wallet balance 0x742d35Cc6634C0532925a3b844Bc9e7595f...
  xclaw02 probe https://api.example.com/paid-endpoint
  xclaw02 pay https://api.example.com/paid-endpoint --max-amount 0.10
...
function getPrivateKey(options) {
  return options['private-key'] ||
         process.env.XCLAW02_PRIVATE_KEY ||
         process.env.PRIVATE_KEY;
}

function getNetwork(options) {
  return options.network ||
         process.env.XCLAW02_NETWORK ||
         process.env.NETWORK ||
         'base';
}
...
function getOpenClawSkillDir() {
  ...
  return path.join(homeDir, '.openclaw', 'skills', 'xclaw02');
}
...
async function cmdWalletBalance(address, options) {
  if (!address) {
    console.error('Error: Address required. Usage: xclaw02 wallet balance <address>');
    process.exit(1);
  }
...
async function cmdProbe(url, options) {
  if (!url) {
    console.error('Error: URL required. Usage: xclaw02 probe <url>');
    process.exit(1);
  }
  ...
  if (options.json) {
    ...
    console.log('✅ URL supports xclaw02 payments\n');
...
async function cmdPay(url, options) {
  if (!url) {
    console.error('Error: URL required. Usage: xclaw02 pay <url>');
    process.exit(1);
  }

  const network = getNetwork(options);
  const maxAmount = options['max-amount'] ||
                    process.env.XCLAW02_MAX_AMOUNT ||
  ...
  if (!privateKey) {
    console.error('Error: Private key required. Use --private-key or set XCLAW02_PRIVATE_KEY');
    process.exit(1);
  }

  if (!maxAmount) {
    console.error('Error: Max amount required. Use --max-amount or set XCLAW02_MAX_AMOUNT');
    process.exit(1);
  }
...
async function cmdFacilitator(options) {
  const url = process.env.XCLAW02_FACILITATOR || DEFAULT_FACILITATOR;
...
### Check balance
\`\`\`bash
npx xclaw02 wallet balance ${config.address}
\`\`\`

### Probe a URL for x402 support
\`\`\`bash
npx xclaw02 probe <url>
\`\`\`

### Make a payment
\`\`\`bash
XCLAW02_PRIVATE_KEY=<key> npx xclaw02 pay <url> --max-amount 0.10
\`\`\`
...
const signer = await createSigner('base', process.env.XCLAW02_PRIVATE_KEY);
...
signer = create_signer('base', os.environ['XCLAW02_PRIVATE_KEY'])
...
- SDK (pip): https://pypi.org/project/xclaw02
