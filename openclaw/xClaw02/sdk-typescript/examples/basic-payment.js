...
async function main() {
  if (!PRIVATE_KEY) {
    ...
    console.error('');
    console.error('Usage:');
    console.error('  PRIVATE_KEY=0x... NETWORK=base MAX_AMOUNT=0.10 API_URL=https://example.com/api/paywall node basic-payment.js');
    console.error('');
  ...
  if (!API_URL) {
    ...
    console.error('Example: API_URL=https://example.com/api/paywall');
    process.exit(1);
  }
